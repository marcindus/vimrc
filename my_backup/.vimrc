...
"explore buffers in quick fix window
command! B call setqflist(map(filter(range(1, bufnr('$')), 'buflisted(v:val)'), '{"bufnr": v:val}'))
...
"explore buffers in quick fix window
nmap <leader>ff :call setqflist(map(filter(range(1, bufnr('$')), 'buflisted(v:val)'), '{"bufnr": v:val}'))<cr>
"no escape
inoremap jj <Esc>
